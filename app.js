...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Initialize folder dragging
    document.querySelectorAll('.mode-folder').forEach(folder => {
        folder.dataset.isFolder = "true";
...
class DinoRunner {
    ...
    reset() {
        this.dino.y = this.height - 30 - this.dino.height;
        this.dino.vy = 0;
    ...
    draw() {
        ...
        // Draw Premiere Pro logo instead of Dinosaur
        ctx.fillStyle = '#391a4d'; // Premiere purple background
        ctx.fillRect(this.dino.x, this.dino.y, this.dino.width, this.dino.height);
        
        ctx.strokeStyle = '#000';
        ctx.lineWidth = 1.5;
        ctx.strokeRect(this.dino.x, this.dino.y, this.dino.width, this.dino.height);
        
        ctx.strokeStyle = '#cc66ff'; // Bright purple inner border
        ctx.lineWidth = 1;
        ctx.strokeRect(this.dino.x + 2, this.dino.y + 2, this.dino.width - 4, this.dino.height - 4);
        
        ctx.fillStyle = '#cc66ff'; // Bright purple "Pr" text
        ctx.font = '900 11px "Outfit", sans-serif';
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
        ctx.fillText('Pr', this.dino.x + this.dino.width / 2, this.dino.y + this.dino.height / 2 + 1);
        ctx.textAlign = 'start';
...
function setupIconDragging(icon) {
    ...
    icon.ontouchstart = dragTouchStart;
    ...
    function closeDragElement() {
        ...
        if (!dragged) {
            if (icon.dataset.isFolder === "true") {
                const mode = icon.id.replace('folder-', '');
                switchMode(mode);
            } else {
                const projId = icon.dataset.projectId;
                if (projId) {
